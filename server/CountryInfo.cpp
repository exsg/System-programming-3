...
void CountryInfo::print() {
    ...
    while (temp != NULL) {
        if (Server::workerThreads == NULL) {
            cout << " -" << temp->name << endl;
        } else {
            cout << " -" << temp->name << " " << Server::workerThreads[temp->worker_number].tid << endl;
        }
        temp = temp->nextCountryInfo;
    }
}
...
WorkerThread * CountryInfo::getWorker(WorkerThread * workers, string country) {
    CountryInfo *temp = countriesHead;
